...
public class SVDItemScorer extends AbstractItemScorer {
    ...
    public ResultMap scoreWithDetails(long user, @Nonnull Collection<Long> items) {
        ...
        Map<Long,Double> userScore_itemId_Value  = Maps.newHashMap();

        for(Long curItem : items)
        {
            RealVector itemFeatures = model.getItemVector(curItem);
            RealVector weightVector = model.getFeatureWeights();
            RealVector temp = weightVector.ebeMultiply(itemFeatures);
            Double prediction = userFeatures.dotProduct(temp);

            Double b = baseline.getIntercept()+baseline.getItemBias(curItem)+baseline.getUserBias(user);

            Double score = b+prediction;
            userScore_itemId_Value.put(curItem, score);

            if(score!=null)
            {
                results.add(Results.create(curItem,userScore_itemId_Value.get(curItem)));
            }
