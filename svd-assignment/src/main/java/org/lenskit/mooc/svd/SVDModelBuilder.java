...
public class SVDModelBuilder implements Provider<SVDModel> {
    ...
    public SVDModel get() {
        ...
        if (featureCount > 0)
        {
            ...
            userMatrix.getSubMatrix(0,userIndex.size()-1,0,featureCount);
            itemMatrix.getSubMatrix(0,itemIndex.size()-1,0,featureCount);
            weights.getSubVector(0, weights.getDimension()-1);
        }
    ...
    private RealMatrix createRatingMatrix(KeyIndex userIndex, KeyIndex itemIndex) {
                                               ...
                                               .stream())
        {
            ...
            for(Rating r : ratings)
            {
                ...
                int row = userIndex.getIndex(r.getUserId());
                int col = itemIndex.getIndex(r.getItemId());

                value = value-( baseline.getIntercept()+baseline.getUserBias(r.getUserId())+baseline.getItemBias(r.getItemId()) );
